...
| 구분 | A (500m) | B (3km) |
|---|---|---|
| 학습 환경 | 500×500 m 단일 Mesh | 3×3 km Large Mesh |
| 핵심 | 상태 다양성 | **지형 다양성** |
...
> **고정된 환경에서 다양한 시작 상태를 학습**

- 500 × 500 m Terrain
- 미리 구성된 여러 GT Path(경로)
- 각 Env는 특정 Path 담당
- Random : Spawn / 속도 / 오프셋 / 헤딩
- 동일한 지형·경로에서 **상태 변화**에 대한 강건성 확보
...
| Path 다양성 | O | O |
| **Terrain 다양성** | 제한적 | **높음** |
| 핵심 차이 | 500×500 한정 | **3×3 km 다양한 지형** |
...
- **많은 Env를 반복하는 것보다 다양한 Path/지형에서 경험을 얻는 것이 더 효율적**임
