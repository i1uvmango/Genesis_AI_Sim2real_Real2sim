...
* 외란 주행에 대해서 더 많은 데이터 추가의 필요성이 보임

## 용어

| 용어 | 정의 |
|---|---|
| **기존** | 구 테이블 — 속도 **−4~4 m/s** 만 실측. 그 위는 v=4 값으로 외삽 |
| **수정** | 재측정 테이블 — 속도 **−4~26 m/s**|
| **envelope / cap** | 횡가속 한계로 정한 조향 상한(feasible 한 속도 한계)|
| **a_lat** | envelope 이 허용하는 횡가속 (m/s²). 이 값이 cap 을 정한다 |
| **K_W** | heading error를 목표 yaw rate 로 바꾸는 게인(피드백) |
---
...
|---|---|---|---|
| v_init (m/s) | v1 −4~4 → **v2 −4~26** | 11 | **21** |
| steer (정규화) | −1 ~ 1 | 5 | **11** |
...
* 신규 장척 10씬. 빨강은 p178·p179 둘뿐
       * 179 는 3D mesh에 의해 차량이 공중에 뜸 &rarr; 물리적으로 못줄이는 잔차
...
| | 기존 32씬 | 신규 장척 10씬 | 외란 28씬 |
|---|---|---|---|
| base 단독 | 3.9 cm | 91 cm (10/10) | — |
| **+ 잔차 it300** | 5.2 cm | **52.9 cm (10/10)** | **완주 24/28** |
...
|---|---|---|
| 완주 | **32/32** | **28/32** |
| 평균 CTE | **5.2 cm** | 49 cm (recovery 제외 구간) |
| 중앙 CTE | 4.2 cm | — |
| 씬별 최대 이탈 평균 | 12.8 cm | 5.93 m |
| 전 씬 최대 이탈 | 0.58 m | 19.81 m |
| recovery 진입 | 없음 | 25/32 씬 · 평균 569 f |

**외란 32씬 중 28 완주, 25씬이 recovery 를 거쳐 복귀.** 미완주 4씬은 recovery 가 경로로 되돌리지 못한 경우다. 무외란 대비 평균 CTE 가 5.2 → 49 cm 로 벌어지는데, 이는 spin 직후 이탈 구간이 평균에 들어간 결과다.
...
RL_recovery 확인 후 장애물 회피 훈련(진행 보류 했던 것 수정 및 재개)